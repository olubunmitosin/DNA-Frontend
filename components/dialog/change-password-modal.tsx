...
export default function ChangePasswordModal({ trigger, user }: Props) {
  ...
  const handleSubmit = async (values: z.infer<typeof SetupPasswordCSchema>) => {
    ...
    startTransition(() => {
      if (user.email) {
        RestPassword(values, user.email).then((data) => {
          // if (data?.invalid) {
  ...
  const handleConfirmOTP = async (email: string, otp: string) => {
    try {
      startTransition(async () => {
        await confirmPasswordOTP(email, otp).then((data) => {
          if (data?.error) {
            toast(data.error);
          }
            ...
            <div>
              <DialogHeader>
                ...
                <div className="mt-2 space-y-3">
                  ...
                  <Button
                    disabled={isPending}
                    onClick={() => handleConfirmOTP(user.email, OTP)}
                    type="button"
